...
export default class BuilderService {
  ...
  constructor(assets) {
    ...
    this.assetsPath = assets.conf.assetsPath;
    this.uploadPath = assets.conf.uploadPath;
    this.deletePath = assets.conf.deletePath;
    this.newDirectoryPath = assets.conf.newDirectoryPath;
    this.canManageFolders = assets.conf.canManageFolders;
  }
  ...
  setListeners() {
    ...
    this.editor.on("modal:open", (modal) => {
    ...
    this.editor.on("asset:open", (modal) => {
      if (this.assetsLoaded) return;

      const AssetManager = this.getEditor().AssetManager;
      const AssetManagerProps = AssetManager.getAssetManagerProps();
      const DirectoriesCollection = AssetManager.getDirectories();
      
      AssetManagerProps.set({
        assetsPath: this.assetsPath,
        newDirectoryPath: this.newDirectoryPath,
        canManageFolders: this.canManageFolders,
      });

      setTimeout(() => {
        fetch(this.assetsPath, {
          headers: {
          ...
          .then(({ assets, directories }) => {
            ...
            DirectoriesCollection.reset(directories);
            AssetManagerProps.set({ isLoading: false });
  ...
  initEmailMjml() {
    ...
    this.editor = grapesjs.init({
      ...
      assetManager: this.getAssetManagerConf(),
      plugins: [grapesjsmjml, grapesjspostcss, grapesjsmautic, ckeditorPlugin],
      pluginsOpts: {
        grapesjsmjml: {},
