...
function switchBuilderButton(theme) {
  ...
  const mEmailBuilderButton = mQuery(
    "#emailform_buttons_builder_toolbar_mobile"
  );
  ...
  function childInitSelectTheme(themeField) {
    ...
    if (builderUrl.length) {
      if (builderUrl.val().indexOf("pages") !== -1) {
        url = builderUrl
          .val()
          .replace("s/pages/builder", "s/grapesjsbuilder/page");
      } else {
        url = builderUrl
          .val()
          .replace("s/emails/builder", "s/grapesjsbuilder/email");
      }
