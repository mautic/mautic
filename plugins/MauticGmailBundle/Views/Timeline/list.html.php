...
<!-- timeline -->
<div class="event-list"  id="contact-timeline">
        ...
        <?php foreach ($events['events'] as $counter => $event): ?>
            ...
            if ($newCount-- > 0) { echo " tr-new"; }
            ?>">

                <span class="timeline-row-id hide"><?=$event['timestamp']->format('U')?></span>
                <span class="timeline-row-lead-id hide"><?=$event['leadId']?></span>
                <span class="timeline-icon">
                    <a href="javascript:void(0);" onclick="jQuery('#timeline-details-<?php echo $counter; ?>').toggleClass('hide')" data-activate-details="<?php echo $counter; ?>" class="btn btn-sm btn-nospin btn-default<?php if (empty($details)) {
                        echo " disabled";
                ...
                <span class="timeline-lead ellipsis"><a href="mailto:<?php echo $event['leadEmail']; ?>" title="<?php echo $event['leadEmail']; ?>" target="_new"><?php echo $event['leadName']; ?></a></span>
