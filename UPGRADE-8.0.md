...
- Deprecated method `Mautic\CoreBundle\Controller\FormController::setStandardParameters()` removed, together with its backing `deprecated*` properties. Controllers extending `FormController` must now override the individual getters (`getModelName()`, `getPermissionBase()`, `getRouteBase()`, `getSessionBase()`, `getTranslationBase()`, `getTemplateBase()`, `getJsLoadMethodPrefix()`) directly instead of calling `setStandardParameters()` from the constructor, as the rest of the standard controllers already do.
