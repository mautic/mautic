...
- Deprecated method `Mautic\CoreBundle\Doctrine\Mapping\ClassMetadataBuilder::addLead()` removed. Use `addContact()` instead, which maps the `contact_id` join column. The method had no callers in core; any custom entity mapping still calling `addLead()` (which maps a legacy `lead_id` column) must switch to `addContact()` and migrate the column from `lead_id` to `contact_id`.
