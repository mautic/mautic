...
/**
 * Show/hide interval settings and clear day-specific values when not applicable
 */
Mautic.campaignEventShowHideIntervalSettings = function() {
    ...
    if (unit === 'i' || unit === 'h') {
        ...
        mQuery('#campaignevent_triggerRestrictedStartHour').val('');
        mQuery('#campaignevent_triggerRestrictedStopHour').val('');
        mQuery('[id^="campaignevent_triggerRestrictedDaysOfWeek_"]').prop('checked', false);
    } else {
...
Mautic.campaignToggleTimeframes = function() {
    var selectedMode = mQuery('input[name="campaignevent[triggerMode]"]:checked').val();
    var triggerSections = {
        interval: '#triggerInterval',
        date: '#triggerDate',
        optimized: '#triggerOptimized'
    };
    
    // Hide all trigger sections
    mQuery('#triggerInterval, #triggerDate, #triggerOptimized').addClass('hide');
    
    // Show the selected section
    if (triggerSections[selectedMode]) {
        mQuery(triggerSections[selectedMode]).removeClass('hide');
    }

    // Clear day-specific settings when not in interval mode
