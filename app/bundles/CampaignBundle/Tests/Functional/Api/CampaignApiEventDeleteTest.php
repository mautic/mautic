...
final class CampaignApiEventDeleteTest extends MauticMysqlTestCase
{
    public function testEventAndSourceDeleteViaPutReproducesApiBug(): void
    {
        ...
        $this->client->request(Request::METHOD_GET, '/api/campaigns');
        $this->assertResponseIsSuccessful();

        // Step 1: GET the campaign (like API Library test does)
        $this->client->request(Request::METHOD_GET, "/api/campaigns/{$campaign->getId()}");
        $this->assertResponseIsSuccessful();
        ...
        $this->client->request(Request::METHOD_PUT, "/api/campaigns/{$campaign->getId()}/edit", $payload);
        $this->assertResponseIsSuccessful();
    }
}
