...
class EventModel extends CommonFormModel
{
    ...
    public function triggerEvent($type, $eventDetails = null, $channel = null, $channelId = null)
    {
        ...
        foreach ($events as $campaignId => $campaignEvents) {
            if (empty($campaigns[$campaignId])) {
                $this->logger->debug("CAMPAIGN #{$campaignId}: Campaign entity for ID# ".$campaignId.' not found');
            ...
            foreach ($campaignEvents as $k => $event) {
                ...
                if (!$response = $this->invokeEventCallback($event, $decisionEventSettings, $lead, $eventDetails, $systemTriggered)) {
                    $this->logger->debug("CAMPAIGN #{$campaignId}: ".ucfirst($event['eventType']).' ID# '.$event['id'].' callback check failed with a response of '.var_export($response, true));
    ...
    public function executeEvent(
        ...
        if ($eventTriggerDate instanceof \DateTime) {
            ...
            if ($response instanceof LeadEventLog) {
                ...
                $result = false;
                $debug  = "CAMPAIGN #{$campaignId}: ".ucfirst($event['eventType']).' ID# '.$event['id'].' for contact ID# '.$lead->getId().' failed with a response of '.var_export($response, true);
    ...
    public function handleCondition(
        ...
        $this->logger->debug(
            "CAMPAIGN #{$campaignId}: Condition ID# ".$event['id'].' triggered with '.$decisionPath.' decision path. Has '.count($childEvents).' child event(s).'
        );
                ...
                foreach ($nonActionEvents as $parentId => $events) {
                    ...
                    foreach ($leads as $lead) {
                        ...
                        if (!isset($leadLog[$lead->getId()]) || !array_key_exists($parentId, $leadLog[$lead->getId()])) {
                            ...
                            $this->logger->debug(
                                "CAMPAIGN #{$campaignId}: Processing the following events for contact ID# ".$lead->getId().': '.implode(', ', array_keys($eventTiming))
                            );
