        ...
        if (!empty($name)) {
            ...
            if ($this->request->query->has('filterby')) {
                ...
                if ($value == '') {
                    ...
                    if ($nameParts[1] === 'report') {
                        /** @var Report $reportEntity */
                        $reportEntity = $this->get('doctrine.orm.entity_manager')->getRepository(Report::class)->find($nameParts[2]);

                        $reportFilters = $reportEntity->getFilters();
                        $filterDetails = null;

                        foreach ($reportFilters as $reportFilter) {
                            if ($reportFilter['column'] === $filter) {
                                $filterDetails = $reportFilter;
                                break;
                            }
                        }

                        if ($filterDetails !== null) {
                            $condition = $filterDetails['condition'];
