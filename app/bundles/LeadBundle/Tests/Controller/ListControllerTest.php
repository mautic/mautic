...
final class ListControllerTest extends MauticMysqlTestCase
{
    ...
    {
        $this->client->request(Request::METHOD_GET, '/s/segments?search=has%3Aresults&tmpl=list');
    ...
    public function testSegmentContactGrid(): void
    {
        ...
        $contacts = $this->createContacts();
        $segment  = $this->addContactsToSegment($contacts, 'MySeg');
        $this->client->request(Request::METHOD_GET, sprintf('/s/segment/view/%d/contact/%d', $segment->getId(), $pageId));
        $response = $this->client->getResponse();
        self::assertResponseIsSuccessful();
    ...
    public function testCloneSegmentPage(): void
    {
        ...
        $this->client->request(Request::METHOD_GET, sprintf('/s/segments/clone/%d', $list->getId()));
    ...
    public function testSegmentSearchFilters(): void
    {
        ...
        $this->client->request(Request::METHOD_GET, '/api/segments?search=filters_field:custom_field_test');
        $clientResponse = $this->client->getResponse();
        $this->assertResponseIsSuccessful();
