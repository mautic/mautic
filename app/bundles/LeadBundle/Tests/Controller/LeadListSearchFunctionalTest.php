...
class LeadListSearchFunctionalTest extends MauticMysqlTestCase
{
    ...
    public function testSegmentSearch(): void
    {
        ...
        $this->assertQueries([
            ...
            "SELECT COUNT(l.id) as count FROM {$prefix}leads l USE INDEX FOR JOIN ({$prefix}lead_date_added) LEFT JOIN {$prefix}users u ON u.id = l.owner_id WHERE (EXISTS(SELECT 1 FROM {$prefix}lead_lists_leads lla WHERE (l.id = lla.lead_id) AND (lla.manually_removed = 0) AND (lla.leadlist_id IN (0)))) AND (l.date_identified IS NOT NULL)",
        ], $previousQueries);
