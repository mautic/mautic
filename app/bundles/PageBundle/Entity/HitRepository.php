...
class HitRepository extends CommonRepository
{
     ...
     * @return array
     */
    public function prepareHitsGraphDataBefore($amount = 30, $unit = 'D')
    {
        $isTime = '';
     ...
     * @return array
     */
    public function prepareHitsGraphDataAfter($data, $downloads, $unit)
    {
        // Group hits by date
        foreach ($downloads as $download) {
            if (is_string($download['dateHit'])) {
                $download['dateHit'] = new \DateTime($download['dateHit']);
            }

            $oneItem = $download['dateHit']->format($this->getDateLabelFromat($unit));
