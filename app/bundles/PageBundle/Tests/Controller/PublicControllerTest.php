...
class PublicControllerTest extends MauticMysqlTestCase
{
    ...
    public function testIndexActionWithAnalyticsAndFooterScripts(): void
    {
        $pageEntity = $this->createMock(Page::class);
        $pageEntity->method('getId')->willReturn(1);
        $pageEntity->method('isPublished')->willReturn(true);
        $pageEntity->method('getVariants')->willReturn([$pageEntity, []]);
        $pageEntity->method('getVariantHits')->willReturn(0);
        $pageEntity->method('getTranslations')->willReturn([]);
        $pageEntity->method('isTranslation')->willReturn(false);
        $pageEntity->method('getContent')->willReturn(null);
        ...
        $pageEntity->method('getIsPreferenceCenter')->willReturn(false);
        $pageEntity->method('getRedirectType')->willReturn(null);
        ...
        $assetHelper     = new AssetsHelper($packagesMock);
        $mauticSecurity  = $this->createMock(CorePermissions::class);
        $mauticSecurity->method('hasEntityAccess')->willReturn(false);

        $pageModel = $this->createMock(PageModel::class);
        $pageModel->method('getHitQuery')->willReturn([]);
        $pageModel->method('getEntityBySlugs')->willReturn($pageEntity);
        $pageModel->method('hitPage')->willReturn(true);

        $this->contactRequestHelper->method('getContactFromQuery')->willReturn(new Lead());
        ...
        $router = $this->createMock(RouterInterface::class);
        $router->method('generate')->willReturn('/mautic.js');
        $doctrine     = $this->createMock(ManagerRegistry::class);
        $userHelper   = $this->createMock(UserHelper::class);
        $translator   = $this->createMock(Translator::class);
        $flashBag     = $this->createMock(FlashBag::class);
    ...
    public function testPreviewActionWithAnalyticsAndFooterScripts(): void
    {
        $pageEntity = $this->createMock(Page::class);
        $pageEntity->method('getId')->willReturn(2);
        $pageEntity->method('isPublished')->willReturn(true);
        $pageEntity->method('getContent')->willReturn(null);
        $pageEntity->method('getCustomHtml')->willReturn('<html><head></head><body>preview</body></html>');
        $pageEntity->method('getCreatedBy')->willReturn(1);
        $pageEntity->method('isPublicPreview')->willReturn(true);
        $pageEntity->method('hasDraft')->willReturn(false);

        $coreParametersHelper = $this->createMock(CoreParametersHelper::class);
        $coreParametersHelper->method('get')->willReturnMap([
            ['google_analytics', null, '<script>preview-analytics</script>'],
            ['footer_script', null, '<script>preview-footer</script>'],
            ['show_contact_frequency', null, false],
            ['show_contact_pause_dates', null, false],
            ['show_contact_preferred_channels', null, false],
            ['show_contact_categories', null, false],
            ['show_contact_segments', null, false],
        ]);
        ...
        $assetHelper     = new AssetsHelper($packagesMock);
        $mauticSecurity  = $this->createMock(CorePermissions::class);
        $mauticSecurity->method('hasEntityAccess')->willReturn(true);
        $mauticSecurity->method('isAnonymous')->willReturn(false);
        $mauticSecurity->method('isAdmin')->willReturn(true);

        $pageModel = $this->createMock(PageModel::class);
        $pageModel->method('getEntity')->willReturn($pageEntity);

        $modelFactory = $this->createMock(ModelFactory::class);
        $modelFactory->method('getModel')->willReturn($pageModel);

        $pageConfig = new PageConfig($coreParametersHelper);
        ...
        $response = $controller->previewAction(
            $request,
            $pageConfig,
            $mauticSecurity,
            $analyticsHelper,
