...
final class AssetModelTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testTrackDownloadLinksAssetForSystemEntry(): void
    {
        ...
        $request->expects($this->exactly(5))
            ->method('get')
            ->willReturnCallback(function (...$parameters): null {
                $this->assertContains($parameters[0], ['utm_campaign', 'utm_content', 'utm_medium', 'utm_source', 'utm_term']);
        ...
        $this->entityManager->expects($this->once())
            ->method('persist')
            ->with($this->callback(function ($downloadPersist) use (&$persisted): true {
                $persisted = $downloadPersist;

                $this->assertInstanceOf(Download::class, $downloadPersist);
        ...
        $this->entityManager->expects($this->once())
            ->method('detach')
            ->with($this->callback(function ($downloadDetach) use (&$persisted): true {
                $this->assertSame($downloadDetach, $persisted);
        ...
        $this->assertSame($asset, $persisted->getAsset(), 'Download row must link to the asset even on the system-entry path.');
        $this->assertSame($lead, $persisted->getLead());
    }
