...
<div class="text-center">
    ...
    <?php elseif ('pdf' == mb_strtolower($activeAsset->getFileType())) : ?>
        <iframe src="<?php echo $assetDownloadUrl.'?stream=1'; ?>#view=FitH" class="col-sm-12"></iframe>
    <?php elseif (0 === mb_strpos($activeAsset->getMime(), 'video') || in_array($activeAsset->getExtension(), ['mpg', 'mpeg', 'mp4', 'webm'])): ?>
        <video src="<?php echo $assetDownloadUrl.'?stream=1'; ?>" controls>
            <?php echo $view['translator']->trans('mautic.asset.no_video_support'); ?>
        </video>
    <?php elseif (0 === mb_strpos($activeAsset->getMime(), 'audio') || in_array($activeAsset->getExtension(), ['mp3', 'ogg', 'wav'])): ?>
        <audio controls>
