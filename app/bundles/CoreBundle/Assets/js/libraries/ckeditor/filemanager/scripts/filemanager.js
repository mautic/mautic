...
---------------------------------------------------------*/
  ...
  var capabilities =
    ...
    new Array("select", "download", "rename", "move", "delete", "replace");

  // Get localized messages from file
  // through culture var or from URL
  if (
    $.urlParam("langCode") != 0 &&
    file_exists("scripts/languages/" + $.urlParam("langCode") + ".js")
  )
  ...
  var setUploader = function(path) {
    ...
    $("#uploader h1")
      .text(lg.current_folder + displayPath(path))
      .attr("title", displayPath(path, false))
      .attr("data-path", path);
            ...
            $.getJSON(
              ...
              function(result) {
                if (result["Code"] == 0) {
                  ...
                  $("#filetree")
                    .find('a[data-path="' + result["Parent"] + '/"]')
                    .click()
                    .click();
                } else {
                  $.prompt(result["Error"]);
...
---------------------------------------------------------*/
  ...
  var renameItem = function(data) {
    ...
    var getNewName = function(v, m) {
      ...
      if (rname != "") {
        ...
        if (
          data["Path"].charAt(data["Path"].length - 1) != "/" &&
            ...
            if (result["Code"] == 0) {
              ...
              if ($("#fileinfo").data("view") == "grid") {
                $('#fileinfo img[data-path="' + oldPath + '"]')
                  ...
                  .next("p")
                  .text(newName);
                $('#fileinfo img[data-path="' + oldPath + '"]').attr(
                  "data-path",
                  newPath
                );
              } else {
                $('#fileinfo td[data-path="' + oldPath + '"]').text(newName);
                $('#fileinfo td[data-path="' + oldPath + '"]').attr(
                  "data-path",
                  newPath
                );
              }
              ...
              $("#fileinfo")
                .find("button#rename, button#delete, button#download")
                .unbind();
              bindToolbar(data);
    ...
    $("#toolbar").ajaxForm({
      ...
      beforeSubmit: function(arr, form, options) {
        ...
        if (getExtension(newFile) != data["File Type"]) {
          $.prompt(
            lg.ERROR_REPLACING_FILE + " ." + getExtension(data["Filename"])
          );
          return false;
        }
      ...
      success: function(result) {
        ...
        if (data["Code"] == 0) {
          ...
          $("#preview")
            .find("img")
            .hide()
            .fadeIn("slow"); // on right panel
          $("ul.jqueryFileTree")
            .find('li a[data-path="' + fullpath + '"]')
            .parent()
            .hide()
            .fadeIn("slow"); // on fileTree
  ...
  var moveItem = function(data) {
    var finalName = "";
    var msg =
      lg.move + ' : <input id="rname" name="rname" type="text" value="" />';
      ...
      $.ajax({
        ...
        success: function(result) {
          if (result["Code"] == 0) {
            ...
            var rootpath = result["Path"].substring(
              0,
            ...
            $("#uploader h1")
              ...
              .attr("title", displayPath(rootpath, false))
              .attr("data-path", rootpath);
            isDeleted = true;
...
---------------------------------------------------------*/
  ...
  var addFolder = function(parent, name) {
    var newNode =
      '<li class="directory collapsed"><a data-path="' +
      parent +
      name +
      '/" href="#">' +
      name +
    ...
    if (parent != fileRoot) {
      ...
      $("#filetree")
        .find('li a[data-path="' + parent + name + '/"]')
        .attr("class", "cap_rename cap_delete")
        .click(function() {
...
---------------------------------------------------------*/
  ...
  var setMenus = function(action, path) {
    ...
    $.getJSON(
      ...
      function(data) {
        if ($("#fileinfo").data("view") == "grid") {
          var item = $("#fileinfo")
            .find('img[data-path="' + data["Path"] + '"]')
            .parent();
        } else {
          var item = $("#fileinfo")
            .find('td[data-path="' + data["Path"] + '"]')
            .parent();
        }
        ...
        if (data["Code"] == 0) {
          ...
          if (data["Protected"] == 0) {
            $("#fileinfo")
              .find("div#tools")
              .append(
                ' <a id="copy-button" data-clipboardtext="' +
                  url +
                  '" title="' +
            ...
            $("#copy-button").click(function() {
              copyToClipboard(baseUrl + this.dataset.clipboardtext);
              $("#fileinfo")
                .find("div#tools")
  ...
  var getFolderInfo = function(path) {
    ...
    var url =
      ...
      "&mode=getfolder&showThumbs=" +
      config.options.showThumbs +
      "&time=" +
      d.getMilliseconds();
    ...
    $.getJSON(url, function(data) {
      ...
      if (data) {
        ...
        if ($("#fileinfo").data("view") == "grid") {
          ...
          result +=
            '<thead><tr><th class="headerSortDown"><span>' +
            lg.name +
            "</span></th><th><span>" +
            lg.dimensions +
            "</span></th><th><span>" +
      ...
      if (data) {
        ...
        for (key in data) {
          ...
          if (data[key]["File Type"] == "dir") {
            var extraclass =
              data[key]["Protected"] == 0 ? "" : " directory-locked";
            result +=
            ...
            if (config.options.listFiles) {
              var extraclass =
                data[key]["Protected"] == 0 ? "" : " file-locked";
              result +=
                '<li class="file ext_' +
  ...
  $(function() {
    ...
    $("#level-up").click(function() {
      ...
      if (cpath != fileRoot) {
        ...
        $("#fileinfo").data("view", currentViewMode);
        $("#filetree")
          .find('a[data-path="' + cpath + '"]')
          .click(); // we close the previous folder
        getFolderInfo(parent);
      }
    });
    ...
    if (config.upload.multiple) {
      ...
      $("#upload").remove();
      $("#newfolder").before(
        '<button class="btn btn-success btn-xs em" id="upload" name="upload" type="button"><i class="btn-xs fa fa-upload mr-3"></i>' +
          lg.upload +
          "</button>"
      );
      ...
      $("#upload")
        ...
        .click(function() {
          ...
          var fileSize =
            config.upload.fileSizeLimit != "auto"
              ? config.upload.fileSizeLimit
