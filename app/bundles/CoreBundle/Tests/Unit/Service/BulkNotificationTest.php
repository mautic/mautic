...
final class BulkNotificationTest extends TestCase
{
    ...
    public function testNotifications(): void
    {
        ...
        $this->assertCount(0, $this->notificationModelFake->notifications ?? []);

        $this->bulkNotification->flush($deduplicateDateTimeFrom = new \DateTime('-2 days'));

        $this->assertCount(2, $this->notificationModelFake->notifications ?? []);
        ...
        $this->assertCount(3, $this->notificationModelFake->notifications ?? []);
        $this->assertNotification($data1, $this->notificationModelFake->notifications[0] ?? [], $deduplicateDateTimeFrom);
        $this->assertNotification($data2, $this->notificationModelFake->notifications[1] ?? [], $deduplicateDateTimeFrom);
    ...
    {
        $this->assertSame($data[1], $notification[0]);
        $this->assertSame($data[2], $notification[1]);
        $this->assertFalse($notification[2]);
        $this->assertSame($data[3], $notification[3]);
        $this->assertSame($data[4], $notification[4]);
        $this->assertSame($data[5], $notification[5]);
        $this->assertSame($data[6], $notification[6]);
        $this->assertSame($data[0], $notification[7]);
        $this->assertSame($deduplicateDateTimeFrom, $notification[8]);
    }
    ...
    private function createNotificationModelFake(): NotificationModel
    {
        return new class() extends NotificationModel {
            ...
            public function addNotification($message, ?string $type = null, bool $isRead = false, $header = null, $iconClass = null, ?\DateTime $datetime = null, ?User $user = null, ?string $deduplicateValue = null, ?\DateTime $deduplicateDateTimeFrom = null): void
            {
