...
final class CircularDependencyValidatorTest extends MauticMysqlTestCase
{
     ...
     */
    public function testValidateOnInvalid(string $expectedMessage, int $expectedCode, callable $newSegmentFiltersBuilder): void
    {
        ...
        foreach ($newSegmentFiltersBuilder($existingSegments) as $segmentKeyToUpdate => $newFilters) {
    ...
    public function validateDataProvider(): array
    {
        return [
            ...
            'Test segment referenced twice but not circular. Succeeds' => [
                '',
                200,
                fn (array $existingSegments) => [
                    'segment-1' => [
                        [
                            ...
                            'object'     => 'lead',
                            'type'       => 'leadlist',
                            'properties' => ['filter' => [$existingSegments['segment-2']->getId()]],
                            'display'    => null,
                            'operator'   => 'in',
                        ],
                        [
            ...
            'Test deep circular through nested children' => [
                'filters: Circular dependency detected in the segment membership filter. API changed (segment-3) > API changed (segment-2) > Segment 1 > API changed (segment-3). This operation would create an infinite loop. Please double check what you are intending to do.',
                422,
                fn (array $existingSegments) => [
                    'segment-2' => [
                        [
