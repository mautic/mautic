...
++ Execution finished", $output);
    ...
    {
        $result = $this->connection->fetchAll("SHOW COLUMNS FROM {$this->tablePrefix}{$table} WHERE Field = '{$column}'");
        Assert::assertNotEmpty($result, sprintf('Table "%s" is expected to have column "%s".', $table, $column));

        $result = $this->connection->fetchAll("SHOW INDEX FROM {$this->tablePrefix}{$table} WHERE Key_name = '{$this->tablePrefix}{$index}'");
    ...
    {
        $this->connection->exec('DROP TABLE IF EXISTS '.$this->tablePrefix.$table);
    }
}
