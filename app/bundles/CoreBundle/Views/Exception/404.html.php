...
<html>
                    ...
                    <div class="linked"><span><strong><?php echo $previous_count; ?></strong> linked Exception<?php echo $previous_count > 1 ? 's' : ''; ?>:</span>
                        <ul>
                            ...
                            <?php foreach ($exception->getAllPrevious() as $i => $previous) : ?>
                                <li>
                                    <?php echo $previous->getClass(); ?> <a href="#traces-link-<?php echo $i + 1; ?>" onclick="toggle('traces-<?php echo $i + 1; ?>', 'traces'); switchIcons('icon-traces-<?php echo $i + 1; ?>-open', 'icon-traces-<?php echo $i + 1; ?>-close');">&#187;</a>
                                </li>
                            <?php endforeach; ?>
                        </ul>
                    </div>
    ...
    <?php foreach ($exception->toArray() as $position => $e) : ?>
        <?php echo $view->render('TwigBundle:Exception:traces.html.twig', array(
            'exception' => $e,
            'position'  => $position,
    ...
    <?php if ($logger) : ?>
        <div class="block">
            ...
            <div id="logs">
                <?php echo $view->render('TwigBundle:Exception:logs.html.twig', array(
                    'logs' => $logger->getLogs()
                )); ?>
            </div>
        </div>
    ...
    <?php echo $view->render('TwigBundle:Exception:traces_text.html.twig', array(
        'exception' => $exception
    )); ?>
