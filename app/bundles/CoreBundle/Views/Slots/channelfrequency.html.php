...
<?php if (isset($form)) : ?>
    <?php if (isset($form['lead_channels']['subscribed_channels'])) : ?>
        <div class="text-left">
            <label class="control-label label0" style="display: inline"><?php echo $view['translator']->trans(
                    ...
                    <td>
                        <div id="frequency_<?php echo $channel->value; ?>" class="text-left row">
                            ...
                            if ($showContactFrequency && isset($form['lead_channels']['frequency_number_'.$channel->value]) && isset($form['lead_channels']['frequency_time_'.$channel->value])):?>
                                <div class="col-md-6">
                                    <label class="text-muted label1"><?php echo $view['translator']->trans(
                                            $form['lead_channels']['frequency_number_'.$channel->value]->vars['label']
                                        ); ?></label>
                                    <?php echo $view['form']->widget(
                                        $form['lead_channels']['frequency_number_'.$channel->value]
                                    ); ?>
                                    <?php echo $view['form']->label(
                                        $form['lead_channels']['frequency_time_'.$channel->value]
                                    ); ?>
                                    <?php echo $view['form']->widget(
                                        $form['lead_channels']['frequency_time_'.$channel->value]
                                    ); ?>
                            ...
                            <?php if ($showContactPauseDates && isset($form['lead_channels']['contact_pause_start_date_'.$channel->value]) && isset($form['lead_channels']['contact_pause_end_date_'.$channel->value])): ?>
                                <div class="col-md-6">
                                    <label class="text-muted label3"><?php echo $view['translator']->trans(
                                            'mautic.lead.frequency.dates.label'
                                        ); ?></label>
                                    <?php echo $view['form']->widget(
                                        $form['lead_channels']['contact_pause_start_date_'.$channel->value]
                                    ); ?>
                                    <?php echo $view['form']->label(
                                        $form['lead_channels']['contact_pause_end_date_'.$channel->value]
                                    ); ?>
                                    <?php echo $view['form']->widget(
                                        $form['lead_channels']['contact_pause_end_date_'.$channel->value]
                                    ); ?>
        ...
        <tr>
            <td>
                <div id="frequency_email" class="text-left row">
                    <div class="col-xs-6">
                        <label class="text-muted label1"><?php echo $view['translator']->trans(
                                'mautic.lead.list.frequency.number'
                            ); ?></label>
                        <input type="text" class="frequency form-control">
                        <label class="text-muted fw-n frequency-label label2"><?php echo $view['translator']->trans(
                                'mautic.lead.list.frequency.times'
                            ); ?></label>
                    ...
                    <div class="col-xs-6">
                        ...
                        <label class="frequency-label text-muted fw-n label4"><?php echo $view['translator']->trans(
                                'mautic.lead.frequency.contact.end.date'
                            ); ?></label>
                        <input type="date" class="form-control">
                    </div>
                </div>
