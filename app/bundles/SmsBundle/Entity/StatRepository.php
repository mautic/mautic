...
class StatRepository extends CommonRepository
{
    ...
    public function getLeadStats($leadId, array $options = [])
    {
        ...
        if (!empty($options['basic_select'])) {
            ...
            $query->select(
                's.sms_id, s.id, s.date_sent as dateSent, e.name, e.name as sms_name, e.message, e.sms_type as type, s.is_failed as isFailed, s.is_delivered as isDelivered,  s.list_id, l.name as list_name, s.tracking_hash as idHash, s.lead_id, s.details'
            )
