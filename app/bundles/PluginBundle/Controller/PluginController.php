...
class PluginController extends FormController
{
     ...
     */
    public function configAction(Request $request, EntityManagerInterface $em, IntegrationHelper $integrationHelper, LoggerInterface $mauticLogger, string $name, $activeTab = 'details-container', $page = 1)
    {
        if (!$this->security->isGranted('plugin:plugins:manage')) {
            $this->throwAccessDenied();
        }
