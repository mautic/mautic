    ...
    public function onReportBuilder(ReportBuilderEvent $event)
    {
        ...
        $columns              = [
            ...
            'unsubscribed' => [
                ...
                'type'    => 'string',
                'formula' => 'IFNULL(SUM(IF('.$doNotContact.'id IS NOT NULL AND dnc.reason='.DoNotContact::UNSUBSCRIBED.', 1, 0)), 0)',
            ],
            'unsubscribed_ratio' => [
                'alias'   => 'unsubscribed_ratio',
                'label'   => 'mautic.email.report.unsubscribed_ratio',
                'type'    => 'string',
                'formula' => 'IFNULL(ROUND((SUM(IF('.$doNotContact.'id IS NOT NULL AND dnc.reason='.DoNotContact::UNSUBSCRIBED.', 1, 0))/'.$prefix.'sent_count)*100, 1), \'0.0\')',
                'suffix'  => '%',
            ],
            'bounced' => [
                'alias'   => 'bounced',
                ...
                'type'    => 'string',
                'formula' => 'IFNULL(SUM(IF('.$doNotContact.'id IS NOT NULL AND dnc.reason='.DoNotContact::BOUNCED.' , 1, 0)), 0)',
            ],
            'bounced_ratio' => [
                'alias'   => 'bounced_ratio',
                'label'   => 'mautic.email.report.bounced_ratio',
                'type'    => 'string',
                'formula' => 'IFNULL(ROUND((SUM(IF('.$doNotContact.'id IS NOT NULL AND dnc.reason='.DoNotContact::BOUNCED.', 1, 0))/'.$prefix.'sent_count)*100, 1), \'0.0\')',
                'suffix'  => '%',
            ],
            $prefix.'revision' => [
