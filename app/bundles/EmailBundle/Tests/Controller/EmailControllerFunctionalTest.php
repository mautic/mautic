...
final class EmailControllerFunctionalTest extends MauticMysqlTestCase
{
    ...
    public function testSegmentEmailSendWithTokenInFromAddress(): void
    {
        ...
        $messageOne = array_values(array_filter($messages, fn (\Mautic\EmailBundle\Mailer\Message\MauticMessage $message): bool => 'contact@one.email' === $message->getTo()[0]->getAddress()))[0];
        $messageTwo = array_values(array_filter($messages, fn (\Mautic\EmailBundle\Mailer\Message\MauticMessage $message): bool => 'contact@two.email' === $message->getTo()[0]->getAddress()))[0];
