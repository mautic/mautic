    ...
    public function testOnReportBuilderWithEmailSentContext(): void
    {
        ...
        $emailStatsCols = [
            ...
            'unsubscribed' => [
                ...
                'label'   => null,
                'type'    => 'string',
                'formula' => 'IFNULL((SELECT ROUND(SUM(IF(dnc.id IS NOT NULL AND dnc.channel_id=e.id AND dnc.reason=1 , 1, 0)), 1) FROM lead_donotcontact dnc), 0)',
            ],
            'unsubscribed_ratio' => [
            ...
            'bounced' => [
                ...
                'label'   => null,
                'type'    => 'string',
                'formula' => 'IFNULL((SELECT ROUND(SUM(IF(dnc.id IS NOT NULL AND dnc.channel_id=e.id AND dnc.reason=2 , 1, 0)), 1) FROM lead_donotcontact dnc), 0)',
            ],
            'bounced_ratio' => [
                'alias'   => 'bounced_ratio',
                'label'   => null,
                'type'    => 'string',
                'formula' => 'IFNULL((SELECT ROUND((SUM(IF(dnc.id IS NOT NULL AND dnc.channel_id=e.id AND dnc.reason=2 , 1, 0))/e.sent_count)*100, 1) FROM lead_donotcontact dnc), \'0.0\')',
                'suffix'  => '%',
            ],
            'vp.id' => [
                'label' => null,
