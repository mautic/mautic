...
class ReportSubscriberFunctionalTest extends MauticMysqlTestCase
{
    ...
    public function testEmailStatReportWithClickThroughColumnsAndAggregate(): void
    {
        $emails = [
            $this->createEmail('Email 1'),
            $this->createEmail('Email 2'),
        ...
        $report->setName('Email with click through');
        $report->setSource('email.stats');
        $report->setColumns(['e.id', 'e.name', 'click_through_count', 'click_through_rate', 'click_to_open_rate']);
        ...
        $this->assertSame([
            // no., id, name, click_through_count, click_through_rate, click_to_open_rate, COUNT e.id, SUM es.is_read
            ['1', (string) $emails[0]->getId(), 'Email 1', '1', '33.3%', '50.0%', '3', '2'],
            ['2', (string) $emails[1]->getId(), 'Email 2', '1', '33.3%', '100.0%', '3', '1'],
        ], $crawlerReportTable);
