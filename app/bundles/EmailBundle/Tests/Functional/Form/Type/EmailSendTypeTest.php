...
class EmailSendTypeTest extends MauticMysqlTestCase
{
    /**
     * @var array<mixed>
     */
    ...
    public function testCampaignEmailSendWithSendToDnc(bool $sendToDnc, int $expectedEmailCopiesCount): void
    {
        ...
        $email      = $this->createEmail($category);
        ...
        $campaign      = $this->createCampaign($segment, $emailId = (int) $email->getId(), $sendToDnc);
    ...
    private function createEmail(Category $category): Email
    {
        $email = new Email();
        $email->setName('Test');
        $email->setCustomHtml('html');
        $email->setTemplate('empty');
    ...
    private function createCampaign(LeadList $segment, int $emailId, bool $sendToDnc): Campaign
    {
        ...
        $event->setProperties($this->getEventProperties($emailId, $sendToDnc));
     ...
     * @return array<mixed>
     */
    private function getEventProperties(int $emailId, bool $sendToDnc): array
    {
        return [
            'canvasSettings'             => [
                'droppedX' => '760',
