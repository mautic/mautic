...
class EmailRepositoryTest extends TestCase
{
    ...
    public function dataGetEmailPendingQueryForCount(): iterable
    {
        ...
        yield [null, false, [], "SELECT count(*) as count FROM {prefix}leads l LEFT JOIN {prefix}email_stats stat ON (stat.lead_id = l.id) AND (stat.email_id = 5) AND (stat.lead_id IS NOT NULL) LEFT JOIN {prefix}message_queue mq ON (mq.lead_id = l.id) AND (mq.status = 'sent') AND (mq.channel = 'email') AND (mq.channel_id = 5) INNER JOIN {prefix}lead_lists_leads ll ON (ll.lead_id = l.id) AND (ll.leadlist_id IN (22, 33)) AND (ll.manually_removed = :false) LEFT JOIN {prefix}lead_donotcontact dnc ON (dnc.lead_id = l.id) AND (dnc.channel = 'email') LEFT JOIN {prefix}lead_categories lc ON (lc.lead_id = l.id) AND (lc.manually_removed = 1) LEFT JOIN {prefix}emails e ON (e.id = 5) AND (e.category_id = lc.category_id) WHERE (mq.id IS NULL) AND ((dnc.id IS NULL) AND (stat.id IS NULL) AND (lc.id IS NULL)) AND ((l.email IS NOT NULL) AND (l.email <> ''))"];
        yield [[6], false, [16], "SELECT count(*) as count FROM {prefix}leads l LEFT JOIN {prefix}email_stats stat ON (stat.lead_id = l.id) AND (stat.email_id IN (6, 5)) AND (stat.lead_id IS NOT NULL) INNER JOIN {prefix}lead_lists_leads ll ON (ll.lead_id = l.id) AND (ll.leadlist_id IN (22, 33)) AND (ll.manually_removed = :false) LEFT JOIN {prefix}lead_donotcontact dnc ON (dnc.lead_id = l.id) AND (dnc.channel = 'email') LEFT JOIN {prefix}lead_categories lc ON (lc.lead_id = l.id) AND (lc.manually_removed = 1) LEFT JOIN {prefix}emails e ON (e.id = 5) AND (e.category_id = lc.category_id) WHERE ((dnc.id IS NULL) AND (stat.id IS NULL) AND (lc.id IS NULL)) AND ((l.email IS NOT NULL) AND (l.email <> ''))"];
