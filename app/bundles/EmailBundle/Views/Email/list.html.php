 ...
 * @var array                                  $filters
 * @var Mautic\EmailBundle\Entity\Email[]      $items
 * @var int                                    $totalItems
 * @var int                                    $page
 * @var int                                    $limit
 * @var string                                 $tmpl
    ...
    <table class="table table-hover table-striped table-bordered email-list">
        ...
        <?php foreach ($items as $item): ?>
            ...
            <tr>
                <td>
                    ...
                    $customButtons = ($type == 'list') ? [
                        [
                            'attr' => [
                                'data-toggle' => 'ajax',
                                'href' => $view['router']->path('mautic_email_action', ['objectAction' => 'send', 'objectId' => $item->getId()]),
                            ],
                ...
                <td class="visible-md visible-lg"><?php echo $item->getMaxDateSent(); ?></td>
                <td>
                    <div>
                        ...
                        <?php endif; ?>
                        <a href="<?php echo $view['router']->path('mautic_email_action', ['objectAction' => 'view', 'objectId' => $item->getId()]); ?>" data-toggle="ajax">
                ...
                <td class="visible-sm visible-md visible-lg col-stats">
                    <?php if ($type == 'list'): ?>
                    <span class="mt-xs label label-info"><?php echo $view['translator']->trans('mautic.email.stat.leadcount', ['%count%' => $model->getPendingLeads($item, null, true)]); ?></span>
                    <?php endif; ?>
                    <span class="mt-xs label label-warning"><?php echo $view['translator']->trans('mautic.email.stat.sentcount', ['%count%' => $item->getSentCount(true)]); ?></span>
                    <span class="mt-xs label label-success"><?php echo $view['translator']->trans('mautic.email.stat.readcount', ['%count%' => $item->getReadCount(true)]); ?></span>
                </td>
