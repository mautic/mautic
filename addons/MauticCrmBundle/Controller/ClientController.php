...
class ClientController extends FormController
{
    ...
    public function indexAction($application, $page = 1)
    {
        ...
        //set limits
        $limit = $session->get('mautic.crm.limit', $this->factory->getParameter('default_pagelimit'));
        $start = ($page === 1) ? 0 : (($page-1) * $limit);
        if ($start < 0) {
            $start = 0;
        }

        $search = $this->request->get('search', $session->get('mautic.crm.filter', ''));
        $session->set('mautic.crm.filter', $search);
        ...
        $orderBy    = $this->factory->getSession()->get('mautic.crm.orderby', 'e.title');
        $orderByDir = $this->factory->getSession()->get('mautic.crm.orderbydir', 'DESC');
        ...
        if ($count && $count < ($start + 1)) {
            ...
            $session->set('mautic.crm.page', $lastPage);
            $returnUrl   = $this->generateUrl('mautic_crm_client_index', $viewParams);

            return $this->postActionRedirect(array(
                'returnUrl'       => $returnUrl,
                'viewParameters'  => array('page' => $lastPage),
                'contentTemplate' => 'MauticCrmBundle:Client:index',
        ...
        return $this->delegateView(array(
            ...
            'contentTemplate' => 'MauticCrmBundle:Client:list.html.php',
            'passthroughVars' => array(
                'activeLink'     => '#mautic_'.$application.'client_index',
                'mauticContent'  => 'clients',
                'route'          => $this->generateUrl('mautic_crm_client_index', $viewParams)
            )
        ));
    }
    ...
    public function newAction ($application)
    {
        ...
        $page   = $session->get('mautic.crm.page', 1);
        $action = $this->generateUrl('mautic_crm_client_action', array(
            'objectAction' => 'new',
            'application'  => $application
        ));
        ...
        if ($this->request->getMethod() == 'POST') {
                ...
                if ($valid = $this->isFormValid($form)) {
                    ...
                    $this->request->getSession()->getFlashBag()->add(
                        ...
                        $this->get('translator')->trans('mautic.core.notice.created', array(
                            ...
                            '%menu_link%' => 'mautic_crm_index',
                            '%url%'       => $this->generateUrl('mautic_crm_client_action', array(
                                'objectAction' => 'edit',
            ...
            if ($cancelled || ($valid && $form->get('buttons')->get('save')->isClicked())) {
                ...
                return $this->postActionRedirect(array(
                    'returnUrl'       => $this->generateUrl('mautic_crm_client_index', $viewParameters),
        ...
        return $this->delegateView(array(
            ...
            'contentTemplate' => 'MauticCrmBundle:Client:form.html.php',
            'passthroughVars' => array(
                'activeLink'    => 'mautic_'.$application.'client_index',
                'mauticContent' => 'page',
                'route'         => $this->generateUrl('mautic_crm_client_action', array(
                        'objectAction' => 'new',
                        'application'  => $application
                ))
            )
    ...
    public function batchDeleteAction($application) {
        ...
        if ($this->request->getMethod() == 'POST') {
            ...
            if (!empty($deleteIds)) {
                ...
                $flashes[] = array(
                    'type' => 'notice',
                    'msg'  => 'mautic.crm.notice.batch_deleted',
                    'msgVars' => array(
                        '%count%' => count($entities)
                    )
                );
    ...
    public function oAuth2CallbackAction($application, $client)
    {
        ...
        if (!isset($postActionVars['returnUrl'])) {
            $postActionVars['returnUrl'] = $this->generateUrl('mautic_crm_client_objects_index', $viewParams);
        }
        ...
        if (!isset($postActionVars['contentTemplate'])) {
            $postActionVars['contentTemplate'] = 'MauticCrmBundle:Mapper:index';
        }
